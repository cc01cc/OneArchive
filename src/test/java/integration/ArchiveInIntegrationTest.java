...
class ArchiveInIntegrationTest {
    ...
    private void assertRootIndexRecords() throws IOException {
        try (Connection conn = databaseAccessor.getConnection();
             PreparedStatement stmt = conn.prepareStatement("SELECT id, root_path FROM root_index WHERE root_path = ?")) {
    ...
    private void assertDirectoryIndexRecords() throws IOException {
        try (Connection conn = databaseAccessor.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                     "SELECT COUNT(*) as dirCount FROM directory_index d " +
    ...
    private void assertFileIndexRecords() throws IOException {
        try (Connection conn = databaseAccessor.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                     "SELECT f.id, f.file_name, f.file_size FROM file_index f " +
    ...
    private void assertDatabaseRecords() throws Exception {
        ...
        // 验证文件与资源映射表
        try (Connection conn = databaseAccessor.getConnection();
             PreparedStatement stmt = conn.prepareStatement("SELECT COUNT(*) as count FROM file_volume_asset");
             ResultSet rs = stmt.executeQuery()) {
            assertTrue(rs.next());
        ...
        // 验证v_asset视图
        try (Connection conn = databaseAccessor.getConnection();
             PreparedStatement stmt = conn.prepareStatement("SELECT COUNT(*) as count FROM v_asset");
             ResultSet rs = stmt.executeQuery()) {
            assertTrue(rs.next());
            assertTrue(rs.getInt("count") > 0, "v_asset视图应该包含记录");
        }
    }
    ...
    private void assertVolumeProcessing() throws Exception {
        ...
        try (Connection conn = databaseAccessor.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                     "SELECT f.file_name, COUNT(fva.id) as volume_count " +
                             "FROM file_index f " +
                             "JOIN file_volume_asset fva ON f.id = fva.file_id " +
                             "JOIN directory_index d ON f.directory_id = d.id " +
                             "JOIN root_index r ON d.root_id = r.id " +
        ...
        try (Connection conn = databaseAccessor.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                     "SELECT COUNT(*) as count FROM file_volume_asset fva " +
                             "JOIN file_index f ON fva.file_id = f.id " +
                             "JOIN directory_index d ON f.directory_id = d.id " +
                             "JOIN root_index r ON d.root_id = r.id " +
                             "WHERE r.root_path = ? AND f.file_name = ?")) {
            stmt.setString(1, Path.of(TEST_SOURCE_DIR).toAbsolutePath().toString());
