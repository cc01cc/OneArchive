...
use crate::mod_database::schema::{ArchiveMetadata, CreateArchiveMetadataParams};
use crate::mod_database::constants_sql;
...
impl ArchiveMetadataDao {
    ...
    pub fn find_by_id(&self, id: i64) -> SqliteResult<Option<ArchiveMetadata>> {
        self.conn.query_single(constants_sql::archive_metadata::SELECT_BY_ID, params![id])
    }
    ...
    pub fn find_by_uri(&self, uri: &str) -> SqliteResult<Option<ArchiveMetadata>> {
        ...
        self.conn.query_single(constants_sql::archive_metadata::SELECT_BY_URI, params![normalized_uri])
    }

    /// 更新归档元数据状态
    pub fn update_status(&self, id: i64, status: &str) -> SqliteResult<()> {
        self.conn.execute(constants_sql::archive_metadata::UPDATE_STATUS, params![status, id])?;
        Ok(())
    }

    pub fn update(&self, id: i64, params: ArchiveMetadata) -> SqliteResult<()> {
        let normalized_path = Database::normalize_directory_path(&Some(params.archive_uri));

        self.conn.execute(
            constants_sql::archive_metadata::UPDATE,
            named_params! {
                ":id": id,
                ":archive_name": params.archive_name,
                ":archive_uri": normalized_path,
                ...
                ":is_compressed": params.is_compressed,
                ":compressed_algorithm": params.compressed_algorithm,
                ":is_encrypted": params.is_encrypted,
                ":encryption_algorithm": params.encryption_algorithm,
                ":status": params.status,
            },
    ...
    pub fn delete(&self, id: i64) -> SqliteResult<()> {
        self.conn.execute(constants_sql::archive_metadata::DELETE_BY_ID, params![id])?;
        Ok(())
    }

    /// 根据状态查找归档元数据
    pub fn find_by_status_and_root_id(
        &self, root_id: i64, status: Option<&str>,
    ) -> SqliteResult<Vec<ArchiveMetadata>> {
        let result = self.conn.query_multiple(
            constants_sql::archive_metadata::SELECT_BY_STATUS_AND_ROOT_ID,
