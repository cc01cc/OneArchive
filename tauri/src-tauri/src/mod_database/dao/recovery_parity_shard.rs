...
use crate::mod_database::{
    common::DatabaseCommonOperations, queries, schema_recovery::RecoveryParityShard,
};
...
impl RecoveryParityShardDao {
    ...
    pub fn insert_recovery_parity_shard(
        ...
        self.conn.insert_and_get_id(
            queries::recovery_parity_shard::INSERT,
            rusqlite::params![group_id, shard_index, shard_path, shard_hash],
        )
    }

    /// 根据组 ID 查找所有校验分片
    pub fn find_recovery_parity_shards_by_group_id(
        &self,
        group_id: i64,
    ) -> SqliteResult<Vec<RecoveryParityShard>> {
        self.conn.query_multiple(
            queries::recovery_parity_shard::SELECT_BY_GROUP_ID,
            rusqlite::params![group_id],
        )
    }

    /// 根据 ID 查找校验分片
    pub fn find_recovery_parity_shard_by_id(
        &self,
        id: i64,
    ) -> SqliteResult<Option<RecoveryParityShard>> {
        self.conn.query_single(
            queries::recovery_parity_shard::SELECT_BY_ID,
            rusqlite::params![id],
        )
    }
    ...
    pub fn update_recovery_parity_shard_status(&self, id: i64, status: &str) -> SqliteResult<()> {
        self.conn.execute(
            queries::recovery_parity_shard::UPDATE_STATUS,
    ...
    pub fn delete_recovery_parity_shard(&self, id: i64) -> SqliteResult<()> {
        self.conn.execute(
            queries::recovery_parity_shard::DELETE_BY_ID,
            rusqlite::params![id],
        )?;
        Ok(())
    }
