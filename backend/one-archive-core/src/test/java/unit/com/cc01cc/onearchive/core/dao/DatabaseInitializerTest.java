...
package unit.com.cc01cc.onearchive.core.dao;
...
class DatabaseInitializerTest {
    ...
    @Test
    @DisplayName("测试 info_root 表是否正确创建")
    void testRootIndexTableCreation() throws SQLException {
        assertTrue(tableExists("info_root"));

        // 检查表结构
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery("PRAGMA table_info(info_root)")) {

            assertTrue(rs.next());
            assertEquals("id", rs.getString("name"));
            assertTrue(rs.next());
            assertEquals("root_name", rs.getString("name"));
            assertTrue(rs.next());
    ...
    @Test
    @DisplayName("测试 info_directory 表是否正确创建")
    void testDirectoryIndexTableCreation() throws SQLException {
        assertTrue(tableExists("info_directory"));
    }
    ...
    @Test
    @DisplayName("测试 info_file 表是否正确创建")
    void testFileIndexTableCreation() throws SQLException {
        assertTrue(tableExists("info_file"));
    }
    ...
    @Test
    @DisplayName("测试 view_file 视图是否正确创建")
    void testFilesByRootViewCreation() throws SQLException {
        assertTrue(viewExists("view_file"));
    }
    ...
    void testAllTablesAndViewsCreated() throws SQLException {
        String[] expectedTables = {
                "info_root", "info_directory", "directory_tree", "info_file",
                "archive_metadata", "archive_asset", "map_file_asset"
        };
