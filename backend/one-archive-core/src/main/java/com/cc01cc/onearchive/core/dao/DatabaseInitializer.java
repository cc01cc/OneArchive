    ...
    private void createTables(Connection conn) throws SQLException {
        ...
        String createDirectoryIndexTable = """
                CREATE TABLE IF NOT EXISTS info_directory (
                    ...
                    created_at INTEGER NOT NULL DEFAULT (strftime('%s', 'now')),
                    updated_at INTEGER NOT NULL DEFAULT (strftime('%s', 'now')),
                    FOREIGN KEY (root_id) REFERENCES info_root(id),
                    UNIQUE (root_id, directory_path)
                );
                """;
        stmt.execute(createDirectoryIndexTable);
        log.info("已创建 info_directory 表");
        ...
        String createDirectoryTreeTable = """
                CREATE TABLE IF NOT EXISTS directory_tree (
                    ...
                    FOREIGN KEY (ancestor_id) REFERENCES info_directory(id),
                    FOREIGN KEY (descendant_id) REFERENCES info_directory(id)
                );
                """;
        ...
        String createFileIndexTable = """
                CREATE TABLE IF NOT EXISTS info_file (
                    ...
                    FOREIGN KEY (directory_id) REFERENCES info_directory(id)
                );
                """;
        stmt.execute(createFileIndexTable);
        log.info("已创建 info_file 表");
