...
public class DirectoryScanner {
    ...
    public static void scanAndSaveDirectory(Path startPath, DatabaseAccessor databaseAccessor) throws IOException {
        scanAndSaveDirectory(startPath, databaseAccessor, null);
    }

    /**
     * 扫描目录并写入或更新数据库（支持进度回调）
     * <p>
     * 1. 将 rootPath 标记为 更新中
     * 2. 将所有 dir 以及 file 标记为 待删除
     * 3. 扫描目录, 已经存在的子目录更新mtime 然后还原回原来的状态; 已存在的文件还原回原来的状态; 新的文件标记为 未存档
     ...
     * @throws IOException 如果扫描过程中发生IO错误
     */
    public static void scanAndSaveDirectory(Path startPath, DatabaseAccessor databaseAccessor, ProgressCallback callback) throws IOException {
        // 根目录-需要转为绝对路径
        ...
        long[] processedSize = {0}; // 使用数组来解决内部类访问问题
        ...
        Files.walkFileTree(startPath, new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFile(@NonNull Path file, @NonNull BasicFileAttributes attrs) throws IOException {
                ...
                // 更新进度
                processedSize[0] += attrs.size();
                if (callback != null && totalSize > 0) {
                    int progress = (int) ((processedSize[0] * 100) / totalSize);
                    // 确保进度在合理范围内
                    progress = Math.min(progress, 100);
                    ProgressInfo progressInfo = new ProgressInfo(processedSize[0], totalSize, 0, 0, "正在扫描文件: " + fileName);
                    callback.updateProgress(progressInfo);
                }
        ...
        if (callback != null) {
            ProgressInfo progressInfo = new ProgressInfo(totalSize, totalSize, 0, 0, "目录扫描完成");
            callback.updateProgress(progressInfo);
        }
    }
